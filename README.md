...
`build_maze.jl` constructs a nominal trajectory from the vector field `corridor_flow_direction_3d` designed in `vector_field.jl` to navigate a pre-defined maze and with which the metric is modified.
...
The utility functions that are used to run the gain search are contained in the files `gain_sweep.jl`.
...
The script `plotting.jl` contains plotting utitilies, with the defaults plotting the maze, vector field, loaded reference trajectory, simulated trajectories, position and velocity errors, as well as body torque control magnitude.
